...
interface IWTQuestionCardProps {
  question: IWTQuestion;
}

export function IWTQuestionCard({ question }: IWTQuestionCardProps) {
  ...
  return (
    <div
      onClick={() => navigate(`/iwt-question/${question.id}`)}
      ...
      <div className="p-5 rounded-2xl border border-gray-200 bg-white shadow-sm hover:shadow-md transition-shadow">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="px-3 py-2 rounded-xl bg-gray-100">
            <span className="text-sm font-bold text-gray-600 opacity-80">
              {question.marks} marks
            </span>
          </div>
        </div>
        <h3 className="text-base font-bold mb-3 leading-6 tracking-tight line-clamp-3">
          {question.question}
        </h3>
        <div className="mb-3 pt-3 border-t border-gray-100">
          <p className="text-sm text-gray-600 opacity-65 leading-5 line-clamp-2">
            {question.answer.substring(0, 120)}...
          </p>
        </div>
        <div className="mt-2 flex justify-end">
          <span className="text-xs font-semibold text-gray-400 opacity-50">
