...
import { IWTQuestionCard } from '../components/IWTQuestionCard'
import { getIWTQuestionsByCategory } from '../data/iwt-questions'

type Category = 'all' | 'short' | 'medium' | 'long';

export default function IWT2024() {
  const navigate = useNavigate();
  const [selectedCategory, setSelectedCategory] = useState<Category>('all');
  const questions = getIWTQuestionsByCategory(selectedCategory, '2024');
    ...
    <div>
      ...
      <div className="px-5 pt-4 pb-3 bg-gray-50 dark:bg-gray-800">
        <p className="text-base text-gray-600 dark:text-gray-300 opacity-65 font-medium text-center">
          2024 Internet & Web Things Exam Questions & Answers
        </p>
      </div>
      ...
      <div className="p-5 pb-8">
        ...
        {questions.map((question) => (
          <IWTQuestionCard key={question.id} question={question} />
        ))}
      </div>
    </div>
  );
